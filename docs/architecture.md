...
The screen is drawn from the same artwork the flasher ships: the Coretastic wordmark across the top, one row per firmware with its mark and name, an inverted highlight band on the current selection, and a `PRG CHANGE  BOOT n` footer. `scripts/firmware/oled_brand.py` derives the one-bit tiles from `web/src/assets/*.png` into `include/oled_brand.h`; `tests/test_oled_brand.py` fails when the header drifts from the sources. Framebuffer composition (`include/oled_frame.h`, `include/oled_screen.h`) is free of ESP-IDF so `tests/oled_test.cpp` can rasterize every screen. The framebuffer and the I2C payload are static storage because together they exceed the 3584-byte main task stack.
