...
Dual-boot selector and USB flasher for running **MeshCore BLE Companion** and **Meshtastic** on one [Heltec WiFi LoRa 32 V4 OLED](https://resource.heltec.cn/download/WiFi_LoRa_32_V4).
...
> **Status:** the integrated firmware builds and host tests run without a device. Physical OLED, radio, switching, and Android pairing are **not yet validated on hardware** — see [Validation status](docs/validation.md).

## Highlights

- **Selector** boots the last choice with a five-second PRG countdown.
- **Isolated apps** — factory resets, backups, and writes are contained to each app's own flash region.
- **Web flasher** + **Python CLI** drive install, update, backup, restore, and recovery over USB.
- **Release bundles** ship validated images, source archives, and checksums.
...
1. Connect USB via a data cable, with the LoRa antenna attached. To enter ROM recovery, hold **PRG**, tap **RESET**, release PRG.
2. Open the [web flasher](#web-flasher) and connect the device. Install from a release bundle.
3. The first boot defaults to MeshCore; tap PRG to choose Meshtastic.
...
Detailed flow in [Architecture](docs/architecture.md); upgrading dependencies in [Maintenance](docs/maintenance.md).

## Web flasher

The browser flasher ships as part of each release (GitHub Pages) and works from HTTPS or localhost with Web Serial. It verifies the installed partition table and bootloader, writes only validated compatible images, and lets you download the full browser log.
...
scripts/          Python tooling
  ...
  firmware/       build.py, prepare.py, integration.cpp, pio_integration.py
...
tests/            Host + Python tests (select/integration, flash, layout)
docs/             architecture.md, maintenance.md, validation.md
```

## Troubleshooting

- **No USB port** — use a data cable, close serial monitors, enter ROM recovery (PRG + RESET); browser needs HTTPS/localhost and Web Serial.
...
Coretastic is GPL-3.0 (see [LICENSE](LICENSE)). MeshCore is MIT (Scott Powell / rippleradios.com); Meshtastic is GPL-3.0; both remain in the pinned submodules. ESP-IDF, esptool-js (Espressif), and Arduino-ESP32 and other libraries retain their own licenses.
