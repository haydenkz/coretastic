...
- Heltec WiFi LoRa 32 **V4.2 or V4.3 OLED**, ESP32-S3R2, 16 MiB flash and 2 MiB PSRAM. Both revisions use the same firmware; the RF front end is detected at runtime.
- V3, V4 R8, TFT variants, and earlier V4 prototypes are unsupported. Read the board marking: USB cannot identify the PCB family. The flasher checks the ESP32-S3 and physical flash capacity.
...
2. Open the flasher, confirm that the physical board is a supported V4.2/V4.3 OLED, and connect the USB device.
3. Download a full local backup before replacing an existing installation.
4. Confirm **Erase existing settings**, then select **Install Coretastic**. This erases every setting, identity, key, contact, and pairing currently on the device.
...
**Restore:** select the backup, confirm the erase, and restore. Checksums, supported board family, original hardware MAC, and partition layout must match. Restoration overwrites the entire flash. Backups of ordinary upstream installations can be saved, but this utility only restores its own dual-boot layout; use the original firmware's recovery tooling for other layouts.
...
From a source checkout, create a Python 3.11 virtual environment and install `requirements.txt`. `--board` explicitly confirms the supported V4 OLED family.

```sh
python3.11 -m venv .venv
. .venv/bin/activate
python -m pip install -r requirements.txt
python scripts/flash.py backup --port /dev/ttyACM0 --board heltec-v4-oled --file before.ctbackup
python scripts/flash.py install --port /dev/ttyACM0 --board heltec-v4-oled --manifest release/manifest.json --erase
python scripts/flash.py meshcore --port /dev/ttyACM0 --board heltec-v4-oled --manifest release/manifest.json
python scripts/flash.py recovery --port /dev/ttyACM0 --board heltec-v4-oled --manifest release/manifest.json
python scripts/flash.py restore --port /dev/ttyACM0 --board heltec-v4-oled --file before.ctbackup --erase
```
...
- **No OLED:** USB recovery remains available. Confirm the board is a supported V4.2/V4.3 OLED; do not guess R8 power pins.
